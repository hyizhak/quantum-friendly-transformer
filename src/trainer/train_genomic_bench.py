...
sn_model = SpectrallyNormalizedTransformerForClassification(
    d_model=512, nhead=8, d_ff=2048, num_emb=len(vocab), num_classes=2, max_seq_len=256,
    apply_embedding_sn=False,
...
optimizer = torch.optim.Adam(sn_model.parameters(), lr=0.001)
...
for epoch in tqdm(range(20)):
    ...
    torch.save(sn_model.state_dict(), f"./model/sn_model_epoch_{epoch}.pth")
    ...
    for x, y in test_loader:
        y_preds.extend(torch.argmax(sn_model(x), dim=1).tolist())
